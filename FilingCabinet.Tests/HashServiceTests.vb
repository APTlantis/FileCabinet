    ...
    Public Class HashServiceTests
        ...
        Sub ComputeHashesReturnsStableKnownValues()
            Using workspace As New TestWorkspace()
                ...
                Dim hashes = New Global.FilingCabinet.HashService().ComputeHashes(path)
        ...
        Sub ComputeHashesReturnsExpandedKnownValuesForEmptyFile()
            Using workspace As New TestWorkspace()
                ...
                Dim hashes = New Global.FilingCabinet.HashService().ComputeHashes(path, "SHA256,BLAKE3,KangarooTwelve,SHA3-256,MD5,Whirlpool,Skein")
        ...
        Sub ComputeHashesOnlyComputesRequestedActiveHashes()
            Using workspace As New TestWorkspace()
                ...
                Dim hashes = New Global.FilingCabinet.HashService().ComputeHashes(path, "MD5")
        ...
        Sub HashRegistryNormalizesUnknownAndEmptySelectionsToDefaults()
            Assert.AreEqual("SHA256", Global.FilingCabinet.HashRegistry.NormalizeActiveHashes(""))
            Assert.AreEqual("SHA256", Global.FilingCabinet.HashRegistry.NormalizeActiveHashes("not-a-hash"))
            Assert.AreEqual("BLAKE3,MD5,crc32", Global.FilingCabinet.HashRegistry.NormalizeActiveHashes("blake3, md5, crc32, nope"))
            Assert.AreEqual(31, Global.FilingCabinet.HashRegistry.Options.Count)
        End Sub
        ...
        Sub ApplyingComputedHashesPreservesInactiveCatalogValues()
            ...
            Dim hashes = New Global.FilingCabinet.FileHashes With {
                .Sha256 = "new-sha"
            }

            Global.FilingCabinet.HashRegistry.ApplyHashesToArtifact(artifact, hashes)
        ...
        Sub MappedHashesUseExtensibleCatalogDictionary()
            ...
            Dim hashes = New Global.FilingCabinet.FileHashes()
            Global.FilingCabinet.HashRegistry.SetHashValue(hashes, "crc32", "352441c2")

            Global.FilingCabinet.HashRegistry.ApplyHashesToArtifact(artifact, hashes)

            Assert.AreEqual("legacy-sha", artifact.Sha256)
            Assert.AreEqual("352441c2", artifact.Hashes("crc32"))
            Assert.AreEqual("352441c2", Global.FilingCabinet.HashRegistry.GetArtifactHashValue(artifact, "CRC-32/IEEE"))
        End Sub

        <TestMethod>
        Sub DynamicHashSettingsExposeAllRegistryOptions()
            Dim viewModel = New Global.FilingCabinet.MainViewModel()
            Dim settings = viewModel.HashSettingOptions.ToList()
            Dim groups = viewModel.HashSettingGroups.ToList()

            Assert.AreEqual(Global.FilingCabinet.HashRegistry.Options.Count, settings.Count)
        ...
        Sub RecomputingHashesDoesNotDependOnPathOrMutateFiles()
            Using workspace As New TestWorkspace()
                ...
                Dim service = New Global.FilingCabinet.HashService()
        ...
        Private Shared Sub AssertCompatibilityVectors(path As String, expected As Dictionary(Of String, String))
            Dim hashes = New Global.FilingCabinet.HashService().ComputeHashes(path, ExpandedCompatibilityHashes)

            For Each pair In expected
                Assert.AreEqual(pair.Value, Global.FilingCabinet.HashRegistry.GetHashValue(hashes, pair.Key), pair.Key)
            Next
        End Sub
    End Class
End Namespace
