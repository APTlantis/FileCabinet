...
Namespace FilingCabinet.Tests
    ...
    Public Class CliParserTests
        ...
        Sub ParseIngestCommandWithGlobalOptionsAndMode()
            Dim command = Global.FilingCabinet.Cli.CliParser.Parse({"ingest", "--copy", "--catalog", "C:\Temp\catalog.json", "--vault", "K:\Vault", "C:\Temp\a.txt"})

            Assert.IsTrue(command.IsValid)
            Assert.AreEqual("ingest", command.CommandName)
            Assert.AreEqual(Global.FilingCabinet.IngestMode.Copy, command.Mode.Value)
        ...
        Sub ParseSearchCommandCollectsQueryAndFilters()
            Dim command = Global.FilingCabinet.Cli.CliParser.Parse({"search", "firmware", "manifest", "--scope", "missing-preview", "--tag", "release", "--limit", "5", "--json"})
        ...
        Sub ParseUnknownCommandReportsInvalidCommand()
            Dim command = Global.FilingCabinet.Cli.CliParser.Parse({"launch", "rockets"})
        ...
        Sub ParseMutatingCommandRequiresApplyAndYesTogether()
            Dim missingApproval = Global.FilingCabinet.Cli.CliParser.Parse({"repair", "--apply"})
            Dim approved = Global.FilingCabinet.Cli.CliParser.Parse({"repair", "--apply", "--yes"})
        ...
        Sub ParseHelpVersionQuietAndZipSwitches()
            Dim help = Global.FilingCabinet.Cli.CliParser.Parse(HelpArgs)
            Dim version = Global.FilingCabinet.Cli.CliParser.Parse(VersionArgs)
            Dim package = Global.FilingCabinet.Cli.CliParser.Parse(QuietZipPackageArgs)
        ...
        Sub ParseValueOptionRequiresValue()
            Dim command = Global.FilingCabinet.Cli.CliParser.Parse(MissingLimitValueArgs)
        ...
        Sub ApplyValueOptionReportsUnexpectedOption()
            Dim command As New Global.FilingCabinet.Cli.CliCommand()
            Dim method = GetType(Global.FilingCabinet.Cli.CliParser).GetMethod("ApplyValueOption", BindingFlags.NonPublic Or BindingFlags.Static)
