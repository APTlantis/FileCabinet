...
Namespace FilingCabinet.Tests
    ...
    Public Class ShellIngestRequestTests
        ...
        Sub ParseCopyShellVerbKeepsSelectedPaths()
            Dim request = Global.FilingCabinet.ShellIngestRequest.Parse({"--copy", "C:\Temp\artifact.txt"})

            Assert.IsTrue(request.Mode.HasValue)
            Assert.AreEqual(Global.FilingCabinet.IngestMode.Copy, request.Mode.Value)
        ...
        Sub ParseMoveShellVerbKeepsFolderPath()
            Dim request = Global.FilingCabinet.ShellIngestRequest.Parse({"/move", "C:\Temp\batch"})

            Assert.IsTrue(request.Mode.HasValue)
            Assert.AreEqual(Global.FilingCabinet.IngestMode.Move, request.Mode.Value)
            Assert.AreEqual("C:\Temp\batch", request.Paths(0))
        End Sub

        <TestMethod>
        Sub ParseWithoutModeLeavesModeUnsetForDefaultIntake()
            Dim request = Global.FilingCabinet.ShellIngestRequest.Parse({"C:\Temp\artifact.txt"})
