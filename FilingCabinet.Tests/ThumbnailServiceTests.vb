    ...
    Public Class ThumbnailServiceTests
        ...
        Sub ImageThumbnailGenerationCreatesBoundedVaultRelativePng()
            ...
            Try
                ...
                Dim result = New Global.FilingCabinet.ThumbnailService().GenerateForArtifact(artifact, vaultRoot)
                Dim thumbnailPath = Path.Combine(vaultRoot, result.RelativePath)
                Dim dimensions = ReadImageDimensions(thumbnailPath)

                Assert.AreEqual(Global.FilingCabinet.ThumbnailService.GeneratedStatus, result.Status)
        ...
        Sub NonImageArtifactUsesFallbackCardStatus()
            ...
            Try
                ...
                Assert.AreEqual(Global.FilingCabinet.ThumbnailService.FallbackCardStatus, result.Status)
        ...
        Sub MissingGeneratedThumbnailIsDetected()
            ...
            Try
                ...
                Assert.IsTrue(New Global.FilingCabinet.ThumbnailService().IsGeneratedThumbnailMissing(artifact, vaultRoot))
            Finally
                If Directory.Exists(workspace) Then
                    Directory.Delete(workspace, recursive:=True)
                End If
