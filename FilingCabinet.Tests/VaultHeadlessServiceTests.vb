    ...
    Public Class VaultHeadlessServiceTests
        ...
        Sub IngestCopyUpdatesCatalogAndKeepsOriginal()
            ...
            Try
                ...
                Dim result = service.Ingest({source}, Global.FilingCabinet.IngestMode.Copy)
        ...
        Sub IngestReturnsPartialExitCodeWhenARequestedPathIsSkipped()
            ...
            Try
                Dim service = CreateService(workspace)
                Dim result = service.Ingest({source, Path.Combine(workspace, "missing.txt")}, Global.FilingCabinet.IngestMode.Copy)
        ...
        Sub SearchMatchesExtractedText()
            ...
            Try
                Dim service = CreateService(workspace)
                service.Ingest({source}, Global.FilingCabinet.IngestMode.Copy)

                Dim results = service.Search(New Global.FilingCabinet.VaultSearchOptions With {.Query = "deterministic keyword"})
        ...
        Sub HeadlessVerifyHonorsCatalogActiveHashes()
            ...
            Try
                ...
                catalog.Artifacts.Add(New Global.FilingCabinet.ArtifactModel With {
                    ...
                    .Path = storedPath,
                    .RelativePath = Path.GetRelativePath(catalog.VaultRootPath, storedPath),
                    .Md5 = New Global.FilingCabinet.HashService().ComputeHashes(storedPath, "MD5").Md5
                })
                catalogService.Save(catalog)
        ...
        Sub RepairApplyRebindsMovedVaultPathAndPreselectsCandidate()
            ...
            Try
                ...
                Dim hashes = New Global.FilingCabinet.HashService().ComputeHashes(storedPath)
                catalog.Artifacts.Add(New Global.FilingCabinet.ArtifactModel With {
                    .Name = "moved.txt",
        ...
        Private Shared Function CreateService(workspace As String) As Global.FilingCabinet.VaultHeadlessService
            Return New Global.FilingCabinet.VaultHeadlessService(New Global.FilingCabinet.HeadlessOptions With {
                .CatalogPath = Path.Combine(workspace, "catalog.json"),
                .VaultRootPath = Path.Combine(workspace, "vault")
            })
        End Function

        Private Shared Function CreateCatalogService(workspace As String) As Global.FilingCabinet.CatalogService
            Return New Global.FilingCabinet.CatalogService(Path.Combine(workspace, "catalog.json"), Path.Combine(workspace, "vault"))
        End Function

        Private Shared Function TestWorkspace() As String
            Dim workspace = Path.Combine(Path.GetTempPath(), "FilingCabinetCliTests", Guid.NewGuid().ToString("N"))
