# Filing Cabinet — Why SHA-256 and BLAKE3

Filing Cabinet records both SHA-256 and BLAKE3 hashes intentionally.
...
That matters for Filing Cabinet because preservation is not only about what the app can verify internally. It is also about whether an artifact can be compared against outside evidence later:
...
SHA-256 gives Filing Cabinet an interoperability anchor that should remain understandable for a long time.

## BLAKE3 Is The Operational Workhorse

BLAKE3 is modern, fast, and designed for efficient hashing on current machines.

It is especially useful for workflows Filing Cabinet naturally grows into:
...
Filing Cabinet is not trying to pick a favorite hash. It is building a stronger fingerprint.
...
That does not make Filing Cabinet magical. It does make the evidence stronger.

## How Filing Cabinet Uses Them

During ingest and verification, Filing Cabinet computes both hashes for the retained file and stores them in the catalog.
...
SHA-256 remains the best outward-facing comparison value. BLAKE3 remains valuable for fast local verification and future automation. Together they support Filing Cabinet's larger promise: a vault should be able to explain whether a retained artifact is still the same artifact.

## The Tradeoff

Computing two hashes costs more than computing one.

For Filing Cabinet, that cost is acceptable. Ingest and verification are trust-building moments, and the extra work produces a catalog that is more portable, more inspectable, and more resilient as tools and expectations change.
